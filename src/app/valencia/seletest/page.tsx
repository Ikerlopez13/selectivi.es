...
export default function SeletestPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Fetch dynamic content from DB for Valencia
      // Note: We use 'Valencia' as the community name
      const { data: subjData } = await supabase.from('asignaturas').select('*').eq('comunidad', 'Valencia').order('nombre')
      if (mounted && subjData) {
  ...
  const startQuiz = () => {
    ...
    window.location.href = '/valencia/seletest/quiz'
  }
  ...
  return (
    <main className={`min-h-screen flex flex-col pb-24 md:pb-0 transition-colors duration-500 ${!isPremium && !loading ? 'bg-[#FFF9E6]' : 'bg-white'}`}>
          ...
          <div className="w-full max-w-[900px] bg-white border border-black/5 rounded-3xl shadow-2xl p-6 md:p-10 relative z-10">
            <div className="text-center mb-4">
               <div className="flex items-center justify-center gap-3 mb-2">
                 ...
                 <span className="bg-orange-500 text-white text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-widest mt-1">València</span>
               </div>
              <p className="text-gray-700 mt-3">Configura tu práctica de selectividad valenciana</p>
              <p className="text-gray-500 text-sm">Contenido adaptado 100% al currículo autonómico</p>
              <div className="mt-3">
            ...
            {subjects.length > 0 ? (
              <>
                    ...
                    return (
                      <div key={id}>
                        <button 
                          ...
                          className={`w-full px-6 py-4 rounded-2xl border text-center font-medium transition-all ${active ? 'bg-[#FF9500] border-[#FF9500] text-white shadow-lg scale-[1.02]' : 'bg-white hover:bg-gray-50 border-gray-200 text-black'}`}
                        >
                          {subj.nombre}
                        </button>
                            ...
                            {subjTopics.map((t) => {
                              ...
                              return (
                                <label key={t.slug} className="flex items-center gap-2 text-sm cursor-pointer hover:text-black transition-colors">
                                  ...
                                  />
                                  <span className={checked ? 'font-medium text-black' : 'text-gray-600'}>
                        ...
                        {active && !isPremium && (
                          <p className="mt-2 text-xs text-gray-500">Con Standard no puedes filtrar. <a href="/valencia/premium" className="text-[#FF9500] font-medium hover:underline">Hazte Premium</a></p>
                        )}
                      </div>
                    )
                  })}
                ...
                <div className="mt-6 space-y-4">
                  <div className="flex items-center justify-between">
                    <label className="font-medium text-black">Número de preguntas</label>
                    <div className="bg-[#FFF9E6] px-3 py-1 rounded-full text-sm font-bold border border-[#FF9500]/20 text-[#8B5E00]">
                  ...
                  <input
                    ...
                    className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#FF9500] disabled:opacity-30"
                  />
                  <label className="flex items-center gap-3 text-sm cursor-pointer p-2 hover:bg-gray-50 rounded-lg transition-colors text-black">
                    <input 
                      type="checkbox" 
                      className="w-5 h-5 accent-[#FF9500] rounded" 
                      checked={mixSubjects} 
                ...
                <div className="mt-8 hidden md:block">
                  <button 
                    onClick={startQuiz} 
                    disabled={maxQuestions === 0} 
                    className={`w-full rounded-2xl py-4 font-bold text-lg shadow-xl shadow-[#FF9500]/10 transition-all active:scale-[0.98] ${maxQuestions === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-[#FF9500] hover:bg-[#ffae33] text-white'}`}
                  >
                    Comenzar Práctica
                  </button>
                  {maxQuestions === 0 && (
              ...
              <div className="py-20 text-center">
                ...
                <h3 className="text-xl font-bold mb-2 text-black">Estamos preparando Valencia</h3>
                <p className="text-gray-500 max-w-sm mx-auto">
                  La base de datos de preguntas para la Comunitat Valenciana se está cargando. ¡Vuelve en unos minutos!
                </p>
                <div className="mt-8">
                  <a href="/valencia" className="text-[#FF9500] font-bold hover:underline">Volver al inicio</a>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="md:hidden fixed inset-x-0 bottom-0 z-20 bg-white/95 backdrop-blur-md border-t p-4 pb-[calc(env(safe-area-inset-bottom)+12px)] shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
          <div className="max-w-[900px] mx-auto px-2">
            <button 
              onClick={startQuiz} 
              disabled={maxQuestions === 0} 
              className={`w-full rounded-2xl py-4 font-bold text-lg transition-all active:scale-[0.95] ${maxQuestions === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-[#FF9500] hover:bg-[#ffae33] text-white shadow-lg'}`}
            >
              Comenzar
            </button>
          </div>
      ...
      {showGate && !isPremium && subjects.length > 0 && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-[500px] bg-white rounded-3xl border shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="p-6 md:p-8 flex flex-col h-full text-black">
              <div className="flex justify-center mb-6">
                <div className="w-16 h-16 bg-[#FF9500]/10 rounded-2xl flex items-center justify-center">
              ...
              <p className="text-center text-gray-500 mb-8 px-4">Desbloquea el 100% de las preguntas valencianas y los filtros expertos.</p>
              
              <div className="space-y-3 mb-8">
                <div className="flex items-center gap-4 bg-gray-50 rounded-2xl p-4 border border-gray-100">
                  <div className="w-10 h-10 bg-[#FF9500] rounded-xl flex items-center justify-center text-white shrink-0">✨</div>
                  <div>
              ...
              <div className="space-y-3">
                <a href="/valencia/premium" className="w-full flex items-center justify-center bg-[#FF9500] hover:bg-[#ffae33] text-white font-bold rounded-2xl py-4 shadow-xl shadow-[#FF9500]/20 transition-all active:scale-[0.98]">
                  Desbloquear con Premium
                </a>
                <button onClick={() => setShowGate(false)} className="w-full flex items-center justify-center bg-gray-100 hover:bg-gray-200 text-gray-600 font-medium rounded-2xl py-4 transition-all text-black">
