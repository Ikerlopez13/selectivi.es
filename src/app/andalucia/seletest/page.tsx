...
export default function SeletestPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Fetch dynamic content from DB for Andalucía
      const { data: subjData } = await supabase.from('asignaturas').select('*').eq('comunidad', 'Andalucía').order('nombre')
      if (mounted && subjData) {
  ...
  const startQuiz = () => {
    ...
    window.location.href = '/andalucia/seletest/quiz'
  }
          ...
          <div className="bg-white border border-black/5 rounded-2xl shadow-xl p-6 md:p-10">
            <div className="text-center mb-4">
               <div className="flex items-center justify-center gap-3 mb-2">
                 ...
                 <span className="bg-black text-white text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-widest mt-1">Andalucía</span>
               </div>
              <p className="text-gray-700 mt-3">Configura tu práctica multiasignatura</p>
              <p className="text-gray-500 text-sm">Contenido adaptado 100% al currículo andaluz</p>
              <div className="mt-3">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-[500px] bg-white rounded-3xl border shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="p-6 md:p-8 flex flex-col h-full">
              ...
              <p className="text-center text-gray-500 mb-8 px-4">Desbloquea el 100% de las preguntas andaluzas y los filtros expertos.</p>
